...
int last_const = 0;
int pref = 0;
int ifs = 0, ops = 0, fors = 0, whls = 0, mods = 0, logi = 0, logl = 0;
int currIn = 0;
int assign = 0;
bool in_idx = false;
    ...
    if (ulaz == "<primarni_izraz>") {
        ...
        if (v[0] == "L_UGL_ZAGRADA") {
            ...
            if (t.lexp == 1 && checkImp(t.type, Int)) {
                ...
                shared_ptr<Tree> ptr2 = node;
                while (ptr2 != nullptr && ptr2->node != "<izraz_naredba>")
                    ptr2 = ptr2->parent;
                if (ptr2 != nullptr && !findNode(ptr2, "OP_PRIDDRUZI")) {
                    file << "   POP R5" << endl;
                    currScope->all_push--;
                }
                ptr.reset();
...
Type UNARNI_IZRAZ(const shared_ptr<Tree> &node) {
    ...
    if (v[0] == "<postfiks_izraz>") {
        ...
        if (ulaz == "<unarni_izraz>") {
            ...
            shared_ptr<Tree> ptr2 = node;
            while (ptr2 != nullptr && ptr2->node != "<inicijalizator>")
                ptr2 = ptr2->parent;
            if (av == "" && ptr2 == nullptr) {
                file << "   POP R5" << endl;
                currScope->all_push--;
    ...
    if (ulaz == "<cast_izraz>") {
        ...
        if (ulaz == "<cast_izraz>") {
            ...
            if (v[0] == "OP_MOD") {
                ...
                file << "mod_" << mods << endl;
                file << "   CMP R0, R1" << endl;
                // file << "   JP_SGE mods_" << mods << endl;
                file << "   JP_SLT mode_" << mods << endl;
                file << "   SUB R0, R1, R0" << endl;
                file << "   JP mod_" << mods << endl;
                file << "mode_" << mods << endl;
                file << "   PUSH R0" << endl;
                currScope->all_push--;
                mods++;
            }
...
Type LOG_I_IZRAZ(const shared_ptr<Tree> &node) {
    ...
    if (ulaz == "<bin_ili_izraz>") {
        ...
        if (ulaz == "<bin_ili_izraz>") {
            ...
            // currScope->all_push--;
            file << "   JP_EQ logif_" << logi << endl;
            file << "   JP logit_" << logi << endl;
            file << "logif_" << logi << endl;
            file << "   MOVE 0, R0" << endl;
            file << "   JP logie_" << logi << endl;
            file << "logit_" << logi << endl;
            file << "   MOVE 1, R0" << endl;
            file << "logie_" << logi << endl;
            // file << "   PUSH R0" << endl;
            // currScope->all_push++;
            logi++;
            return Type(Int, false);
...
Type LOG_ILI_IZRAZ(const shared_ptr<Tree> &node) {
    ...
    if (ulaz == "<log_i_izraz>") {
        ...
        if (ulaz == "<log_i_izraz>") {
            ...
            // currScope->all_push--;
            file << "   JP_EQ loglt_" << logl << endl;
            file << "   JP loglf_" << logl << endl;
            file << "loglf_" << logl << endl;
            file << "   MOVE 0, R0" << endl;
            file << "   JP logle_" << logl << endl;
            file << "loglt_" << logl << endl;
            file << "   MOVE 1, R0" << endl;
            file << "logle_" << logl << endl;
            // file << "   PUSH R0" << endl;
            // currScope->all_push++;
            logl++;
...
void NAREDBA_PETLJE(const shared_ptr<Tree> &node) {
    ...
    if (v[0] == "KR_WHILE") {
        ...
        file << "   POP R0" << endl;
        file << "   CMP R0, 1" << endl;
        file << "   JP_EQ whln_" << whls << endl;
        file << "   JP_NE whle_" << whls << endl;
        currScope->all_push--;
        ulaz = u[curr]->node;
        v = splitSpaces(ulaz);
        if (v[0] != "D_ZAGRADA")
        ...
        file << "whln_" << whls << endl;
        NAREDBA(u[curr]);
        file << "    JP whl_" << whls << endl;
        file << "whle_" << whls << endl;
        whls++;
    } else if (v[0] == "KR_FOR") {
        curr++;
        ulaz = u[curr]->node;
